...
Use texture aliases inside v2 `.kpeffect` files with a top-level emitter
`"texture"` string.
...
`.kpeffect` files are strict JSON v2 templates. At runtime the engine:

1. Looks up the effect key in `ParticleLibrary`.
2. Resolves any texture alias into a runtime texture id.
3. Applies the resulting primary emitter template to the entity.
...
6. Submits a renderer-facing emitter descriptor. Normal effect particles are
...
- Treat `renderer::submitParticles(...)` and `submitPackedParticles(...)` as
  compatibility APIs for baked producers such as beams. They are reported as
...
atlas/flipbook metadata, emission, lifetime, size, rotation, spawn, motion,
...
velocity. `shading_mode = shell` remains available for billboard impostors that need
  ...
  "emitters": [
    {
      ...
      "spawn": { "shape": "sphere", "box_extents": [0, 0, 0], "radius_min": 0.0, "radius_max": 0.25, "radial_speed_min": 1.0, "radial_speed_max": 3.0 },
