...
1. Stale render instances are never explicitly retired.
   - Evidence:
     - Upsert-only submit path: `src/rendering/renderer/backends/diligent/backend_render.cpp:486`
     - Full instance iteration in render: `src/rendering/renderer/backends/diligent/backend_render.cpp:1553`
     - No per-entity retire in render system: `src/rendering/renderer/render_system.cpp:341`
     - No backend API for removing an instance: `include/karma/rendering/renderer/backend.hpp:41`
   - Risk:
...
2. Mesh/material lifetime leaks on mesh key changes and despawns.
   - Evidence:
     - Mesh replaced without destroying prior mesh handle: `src/rendering/renderer/render_system.cpp:373`
     - Mesh destroy only erases mesh record: `src/rendering/renderer/backends/diligent/backend_mesh.cpp:324`
     - `createMeshFromFile` allocates material records per imported material: `src/rendering/renderer/backends/diligent/backend_mesh.cpp:150`
   - Risk:
     - GPU/CPU resource growth as content changes.

3. Duplicate Assimp imports for the same mesh file.
   - Evidence:
     - Bounds import in render system: `src/rendering/renderer/render_system.cpp:199`
     - Full mesh import in backend: `src/rendering/renderer/backends/diligent/backend_mesh.cpp:86`
   - Risk:
     - Avoidable blocking disk/parse time during first-use hitch windows.

4. Per-frame ECS `view()` allocations across hot loops.
   - Evidence:
     - `view()` allocates and returns `std::vector<Entity>`: `include/karma/world/ecs/world.h:78`
     - Repeated use in render loop and helper loops: `src/rendering/renderer/render_system.cpp:248`, `src/rendering/renderer/render_system.cpp:342`, `src/rendering/renderer/render_system.cpp:410`
     - Similar usage in physics/audio: `src/simulation/physics/physics_system.cpp:74`, `src/media/audio/audio_system.cpp:27`
   - Risk:
     - Frame allocator churn and cache misses at scale.

5. Unsorted rendering path with high per-draw state traffic.
   - Evidence:
     - Iteration over unordered instance map: `src/rendering/renderer/backends/diligent/backend_render.cpp:1553`
     - Per-draw VB/IB binds, CB map, SRB commit, draw: `src/rendering/renderer/backends/diligent/backend_render.cpp:1619`, `src/rendering/renderer/backends/diligent/backend_render.cpp:1666`, `src/rendering/renderer/backends/diligent/backend_render.cpp:1690`
   - Risk:
     - Excess CPU overhead and pipeline/resource bind churn.

6. Per-draw shader variable lookups for env textures.
   - Evidence:
     - `GetVariableByName` in draw lambda each draw: `src/rendering/renderer/backends/diligent/backend_render.cpp:1678`
   - Risk:
     - Avoidable repeated string/hash lookup overhead.

7. Validation-heavy draw flags in normal draw paths.
   - Evidence:
     - Frequent `DRAW_FLAG_VERIFY_ALL`: `src/rendering/renderer/backends/diligent/backend_render.cpp:1498`, `src/rendering/renderer/backends/diligent/backend_render.cpp:1698`, `src/rendering/renderer/backends/diligent/backend_ui.cpp:391`
   - Risk:
     - Non-trivial CPU overhead if left on in release/perf runs.

8. Line debug buffer overflow path does not grow buffer.
   - Evidence:
     - Overflow branch has no reallocation action: `src/rendering/renderer/backends/diligent/backend_render.cpp:1727`
     - Initial capacity fixed to 1024 vertices: `src/rendering/renderer/backends/diligent/backend_render.cpp:637`
   - Risk:
     - Lost debug lines and potential confusion during diagnostics.

9. Culling projection mismatch between submit and render.
   - Evidence:
     - Culling uses fixed camera aspect `16:9`: `src/rendering/renderer/render_system.cpp:259`
     - Actual render uses framebuffer aspect: `src/rendering/renderer/backends/diligent/backend_render.cpp:1312`
   - Risk:
     - Incorrect cull decisions (extra overdraw or missing visible meshes).

10. Unused filesystem checks on submission path.
    - Evidence:
      - `std::filesystem::exists` computed but never used: `src/rendering/renderer/render_system.cpp:354`, `src/rendering/renderer/render_system.cpp:371`
    - Risk:
      - Extra I/O/syscall overhead without behavioral benefit.

11. Full scene-entity synchronization each frame.
    - Evidence:
      - `syncSceneEntities()` scans full entity set every frame: `src/runtime/app/engine_app.cpp:126`
    - Risk:
      - Scales poorly with large entity counts.

12. System graph rebuilds schedule each fixed update.
    - Evidence:
      - `buildOrder()` called every `update()`: `include/karma/world/systems/system_graph.h:36`
      - Dependency reverse lookup scans full node set repeatedly: `include/karma/world/systems/system_graph.h:106`
    - Risk:
      - Scheduler overhead grows with system count.

13. Input mapping update is nested over actions, bindings, and events.
    - Evidence:
      - Held and pressed checks in nested loops: `src/runtime/input/input_system.cpp:66`, `src/runtime/input/input_system.cpp:89`
    - Risk:
...
1. Texture cache exists and avoids redundant image decode:
   - `src/rendering/renderer/backends/diligent/backend_textures.cpp:123`
2. Mesh bounds cache exists:
   - `src/rendering/renderer/render_system.cpp:360`
3. Environment precompute is dirty-gated:
   - `src/rendering/renderer/backends/diligent/backend_render.cpp:655`
