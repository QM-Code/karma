 ...
 public:
  ...
  void onStart() override {
    ...
    materials->registerFromMeshTint(
        "light_receiver", marker_mesh_, math::Color{0.82f, 0.82f, 0.82f, 1.0f});
    materials->registerFromMeshTint(
        "shadow_caster", marker_mesh_, math::Color{0.22f, 0.24f, 0.28f, 1.0f});
  ...
  void spawnLights() {
    if (!unsafe_stress_mode_) {
      ...
      for (int index = 0; index < safe_light_count_; ++index) {
        ...
        materials->registerFromMeshTint(marker_material_key, marker_mesh_, color);
