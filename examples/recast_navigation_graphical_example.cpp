...
void registerPanelMaterial(renderer::MaterialLibrary& materials,
  ...
  materials.registerFromMeshTint(key, mesh_key, tint);
}
...
class RecastNavigationGraphicalExample final : public app::GameInterface {
 ...
 private:
  ...
  Panel& createPanel(PanelKind kind,
    ...
    registerPanelMaterial(*materials, material_key, mesh_key, tint);
    panel.mesh_entity = helpers::spawnMeshAsset(*world, panel.name + " Mesh", mesh_key, panel.mesh_offset);
    world->get<components::MeshComponent>(panel.mesh_entity).material_key = material_key;
