...
class DemoGame : public app::GameInterface {
 public:
  ...
  void onFixedUpdate(float dt) override {
    ...
    if ((reset_down && !reset_down_prev_) || reset_requested_) {
      resetTank();
  ...
  void requestTankReset() {
    reset_requested_ = true;
  }

 private:
  void resetTank() {
    if (!world->isAlive(tank_entity_)) {
      return;
    }
...
int main() {
  ...
  overlay->addFontFaceFile("/home/quinn/Documents/bz3/data/client/fonts/GoogleSans.ttf", true);
  overlay->addFontFaceFile("/home/quinn/Documents/bz3/data/client/fonts/Audiowide.ttf", false);
  overlay->setDocumentFile("examples/assets/rmlui/overlay.rml");
  overlay->setContextReadyCallback([&game](Rml::Context& ctx) {
    if (Rml::DataModelConstructor constructor = ctx.CreateDataModel("demo")) {
      constructor.BindEventCallback(
          "reset_tank",
          [&game](Rml::DataModelHandle /*handle*/, Rml::Event& /*ev*/,
                  const Rml::VariantList& /*args*/) {
            game.requestTankReset();
          });
    }
  });
  ...
  config.window.title = "Karma RmlUi Overlay";
